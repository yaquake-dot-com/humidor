...
final class Application: NSObject, NSApplicationDelegate {
    ...
    private func showNotification(_ notification: NotificationMessage, action: String? = nil) {
        let title = (notification.title ?? NicotineCore.Application.name).trimmingCharacters(in: .whitespaces)
    ...
    func onReportBug() {
        openURI(NicotineCore.Application.issueTrackerURL)
    }

    func onImproveTranslations() {
        openURI(NicotineCore.Application.translationsURL)
    }
