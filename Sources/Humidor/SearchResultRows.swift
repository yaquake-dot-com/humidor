...
enum SearchResultRows {
    ...
    private static func groupSummary(_ treeView: TreeView, row: TreeRow, files: [TreeRow],
        ...
        parts.append(RowCellView.symbolText("doc", text: humanize(fileCount)))
        parts.append(plainText(humanSize(totalSize)))
        parts.append(RowCellView.symbolText("arrow.down", text: treeView.rowValue(row, "speed").string))
...
private final class ResultCellView: RowCellView {
    ...
    private let qualityLabel = RowCellView.detailLabel(width: 130)
    private let lengthLabel = RowCellView.detailLabel(width: 44)
    private let sizeLabel = RowCellView.detailLabel(width: 72)
    ...
    init() {
        ...
        for view in [qualityLabel, lengthLabel, sizeLabel, statusLabel, statusIcon] as [NSView] {
